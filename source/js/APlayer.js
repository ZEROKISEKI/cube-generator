...
class APlayer {
    ...
    constructor(option) {
        ...
        this.target = null;
        this.timeout = null
        this.splitLrc = function (t, u) {
            this.timeout = null;
            if(t.length > 18) {
                this.timeout = setTimeout(() => {
                    this.target.innerText = t.substr(18)
                    t = this.target.innerText
                    if (t.length > 0) {
                        this.splitLrc(t, u)
                    }
                }, 18 * u * 1000)
            ...
            if (this.lrcIndex > this.lrc.length - 1 || currentTime < this.lrc[this.lrcIndex][0] || (!this.lrc[this.lrcIndex + 1] || currentTime >= this.lrc[this.lrcIndex + 1][0])) {
                for (let i = 0; i < this.lrc.length; i++) {
                    if (currentTime >= this.lrc[i][0] && (!this.lrc[i + 1] || currentTime < this.lrc[i + 1][0])) {
                        this.lrcIndex = i;
                        this.lrcContents.style.transform = `translateY(${-this.lrcIndex * 35}px)`;
                        this.lrcContents.style.webkitTransform = `translateY(${-this.lrcIndex * 35}px)`;
                        ...
                        /* let temp = this.lrcContents.getElementsByClassName('aplayer-lrc-current')[0]
                        temp.classList.remove('aplayer-lrc-current'); */
                        /* if (this.text && this.text.length > 18) {
                            temp.innerText = this.text
                        }
                        this.target = this.lrcContents.getElementsByTagName('p')[i]
                        this.target.classList.add('aplayer-lrc-current');
                        if (this.target.innerText.length > 18) {
                            this.text = this.target.innerText;
                            let duration = this.lrc[i + 1] ? this.lrc[i + 1][0] - this.lrc[i][0] : this.audio.duration - this.lrc[i][0];
                            let unit = duration / this.text.length
                            this.splitLrc(this.text, unit)
                        } */
                    }
                }
            }
        ...
        const thumbMove = (event) => {
            ...
            this.updateBar('played', percentage, 'width');
            if (this.option.showlrc) {
                // this.lrcContents.style.display = 'none';
                // this.lrcContents.getElementsByTagName('p')[0].classList.add('hidden');
        ...
        const thumbUp = () => {
            ...
            if (isNaN(this.audio.duration)) {
                ...
                this.playedTime = setInterval(() => {
                    ...
                    if (this.option.showlrc) {
                        // TODO modify display
                        // this.lrcContents.style.display = 'block';
                        // this.lrcContents.getElementsByClassName('aplayer-lrc-current')[0].classList.remove('hidden');
                        this.updateLrc();
                    }
                ...
                if (this.option.showlrc === 1) {
                    ...
                    xhr.onreadystatechange = () => {
                        if (xhr.readyState === 4) {
                            ...
                            this.text = this.lrcContents.getElementsByTagName('p')[0].innerText;
                            this.target = this.lrcContents.getElementsByTagName('p')[0];
    ...
    play(time) {
        ...
        if (this.audio.paused) {
            /* if (this.audio.currentTime === 0) {
                if (this.target.innerText.length > 18) {
                    this.text = this.target.innerText;
                    let duration = this.lrc[1][0] - this.lrc[0][0];
                    let unit = duration / this.text.length
                    this.splitLrc(this.text, unit)
                }
            } */
